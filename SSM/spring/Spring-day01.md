...
- 格式：
  ...
  ```xml
   <!--    实例工厂创建bean的方式-->
  <bean id="factory" class="com.mao.factory.Userfactory"></bean>
      <!--    先创建实例工厂的bean对象，将id给factory-bean,指明工厂实例对象是谁的作用 -->
...
<!--
        constructor-arg标签进行构造方法的注入 name表示属性名
        type 属性的值是这个构造方法进行参数赋值时的参数类型，可以用于校验参数类型等是否正常，甚至可以用于进行按照类型进行给参数赋值
        -->
```
...
1.BeanFactory创建的bean采用延迟加载形式，使用才创建。也就是说第一次需要获取该对象时，才会进行创建，即使是单例的也是如此。
